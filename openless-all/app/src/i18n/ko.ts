...
export const ko: typeof zhCN = {
  ...
  shell: {
    ...
    navHint: {
      ...
      vocab: '사전: 고유명사 인식률을 높이는 사용자 지정 핫워드',
  ...
  onboarding: {
    ...
    hotkeyTitle: '전역 단축키',
    accessibilityDesc:
      '전역 단축키(기본 {{trigger}}) 감지와 인식 결과를 커서 위치에 입력하기 위해 사용합니다.',
    ...
    accessibilityHint:
      '허용 후에는 **OpenLess 를 완전히 종료** 한 다음 다시 실행해야 합니다(macOS TCC 규칙).',
    footerHint:
      '모든 권한이 부여되면 이 가이드는 자동으로 닫힙니다. 닫히지 않으면 메뉴 막대의 OpenLess → 종료 후 앱을 다시 실행해 주세요.',
    continueToSettings: '설정만 열기(음성 및 전역 단축키는 사용 불가)',
    androidContinue: '앱으로 계속',
    androidFooterHint:
      '받아쓰기에는 마이크 권한이 필요합니다. 위의 권한 요청을 탭하거나, 앱으로 먼저 들어가 개요 페이지에서 나중에 허용할 수 있습니다.',
  ...
  overview: {
    refresh: '상태 새로고침',
    servicesTitle: '현재 음성 서비스',
    statsTitle: '사용 기록',
    omniKind: '멀티모달 음성',
    omniName: '현재 Omni 모델',
    statusLoading: '서비스 설정을 불러오는 중…',
    configureProvider: '설정하기',
    manageProvider: '서비스 관리',
    recentEmptyHint:
      '아직 받아쓰기 기록이 없습니다. 위 안내에 따라 사용해 보면 결과가 여기에 표시됩니다.',
    providerHelp: {
      asr: '음성을 텍스트로 변환합니다.',
      llm: '내 스타일에 맞게 글을 정리하고 다듬습니다.',
      omni: '하나의 모델로 음성 인식과 텍스트 처리를 수행합니다.',
    },
    actions: {
      refresh: '다시 불러오기',
    ...
    guide: {
      nextStep: '다음 단계',
      loadingTitle: '설정을 불러오고 있어요',
      loadingDesc: '잠시 후 현재 서비스와 다음 할 일을 보여 드릴게요.',
      unavailableTitle: '서비스 상태를 불러올 수 없어요',
      unavailableDesc: '다시 불러오거나 AI 서비스에서 설정을 확인해 주세요.',
      servicesTitle: '먼저 음성 서비스를 설정하세요',
      servicesDesc:
        '여기서 시작하는 것을 추천해요. 음성 인식과 텍스트 처리 서비스를 선택하세요. Omni 모드에서는 사용할 멀티모달 모델만 설정하면 됩니다.',
      permissionsTitle: '단축키 상태를 확인하세요',
      permissionsDesc:
        '현재 단축키 기능을 사용할 수 없습니다. 권한 및 데이터에서 상태와 해결 방법을 확인해 주세요.',
      shortcutsTitle: '녹음 단축키를 설정하세요',
      shortcutsDesc: '편한 단축키를 선택하면 입력 중에 받아쓰기를 시작할 수 있어요.',
      recordingTitle: '녹음 방식을 확인하세요',
      recordingDesc:
        '서비스 설정이 저장되어 있습니다. 녹음 설정에서 마이크와 원하는 녹음 방식을 선택하세요.',
      tryDictationTitle: '받아쓰기를 해 보세요',
      tryDictationDesc: '입력할 곳에 커서를 놓으세요. {{shortcut}}',
      permissionsHint:
  ...
  history: {
    ...
    stepAsrHint:
      '키를 뗀 후 인식 결과를 기다린 시간. 스트리밍 인식은 녹음 중에 변환하므로 보통 녹음 시간보다 훨씬 짧습니다.',
    stepPolish: '다듬기',
    stepInsert: '삽입',
    chars: '{{count}}자',
    ...
    repolish: {
      ...
      timeout:
        '현재 LLM 제공자가 30초 안에 응답하지 않았습니다. 더 빠른 제공자로 바꾸거나 잠시 후 다시 시도하세요 — 무료 모델 풀은 대기가 잦습니다.',
      resultTitle: '{{name}} 결과',
  ...
  vocab: {
    ...
    newWord: '새 단어',
    newWordTitle: '새 단어 추가',
    newWordDesc: '단어를 직접 입력하거나 프리셋 템플릿에서 일괄 가져오세요.',
    newWordInputPlaceholder: '단어 입력 후 Enter로 추가…',
    ...
    corrections: {
      ...
      invalid:
        '문자 그대로 바꾸기 또는 {num} 숫자 와일드카드 1개가 포함된 규칙만 지원합니다. 예: {num}粒 → {num}例.',
  ...
  style: {
    ...
    customPromptHint:
      '비워 두면 현재 동작이 그대로 유지됩니다. 저장 후 이 스타일의 실시간 다듬기와 repolish 모두에 적용됩니다. Ctrl/Cmd+Enter로도 저장할 수 있습니다.',
    customPromptSave: '프롬프트 저장',
    customPromptDirty: '미저장',
    systemPromptMovedHint:
      '전체 system prompt 편집은 Settings -> Providers 로 이동했습니다. 이 페이지는 이제 스타일 활성화와 기본값만 다룹니다.',
    modes: {
      raw: {
        name: '원문',
        desc: '구두점과 필요한 문장 구분만 보충하고 다시 쓰거나 확장하지 않습니다.',
        sample:
          '원래 구어체 유지. "음", "그게" 같은 입버릇은 제거하지만 문장을 재구성하지 않습니다.',
      },
      light: {
        name: '가벼운 정리',
        desc: '입버릇 제거, 구두점 보충, 자연스럽게 보낼 수 있는 텍스트로 정리합니다.',
        sample:
          '원고를 읽는 듯한 느낌이 들지 않도록 어조와 표현 습관은 남기되, 문장이 매끄럽게 흐르도록 합니다.',
      },
      structured: {
        name: '명확한 구조',
      ...
      formal: {
        name: '정식 표현',
        desc: '업무 커뮤니케이션과 메일에 적합. 더 전문적이고 완성도 높은 문체.',
        sample: '메일 시나리오에서 인사말과 맺음말을 자동 인식. 공허한 상투어는 추가하지 않습니다.',
      },
    },
    pack: {
      builtinTags: {
      ...
      iconInvalid: '외부 리소스가 없는 유효한 SVG 아이콘을 선택하세요(최대 256 KB).',
      iconSaveFailed: '아이콘을 저장하지 못했습니다. 다시 시도하세요.',

      selectionListTitle: '선택 영역 다듬기 스타일',
      selectionListDesc:
        'ASR 없이 선택한 글에 사용: 문법, 명확성, 형식 다듬기. 스타일과 프롬프트를 따로 고를 수 있습니다.',
      ...
      publishBuiltinRejected:
        '기본 팩은 직접 게시할 수 없습니다. 먼저 편집해서 가져오기 버전을 만드세요.',
      builtin: '기본',
      imported: '가져옴',
      active: '사용 중',
      activate: '활성화',
  ...
  translation: {
    ...
    howto: {
      ...
      step3: '녹음 중 {{shortcut}} 를 한 번 눌러 번역 활성화.',
  ...
  selectionAsk: {
    ...
    guide: {
      ...
      unsetDesc: '먼저 단축키 설정에서 선택 질문 단축키를 지정하세요.',
      selectTitle: '궁금한 내용 선택',
      askTitle: '말로 질문하기',
      askDesc: '<key>{{recordHotkey}}</key>로 녹음하고, 다시 눌러 전송하세요.',
  ...
  settings: {
    selectionWorkspace: {
      ...
      voiceDeliveryDesc:
        '음성 편집 후: 선택 영역을 바로 교체하거나 Ask 패널에서 확인 후 교체합니다.',
      voiceEnable: '음성 편집',
      voiceEnableDesc:
        '위와 같은 단축키 사용. 녹음 방식은 전역 설정을 따릅니다(현재: {{recordingLabel}}).',
      autoIntent: '의도 자동 판별',
      autoIntentDesc:
    ...
    codingConsole: {
      ...
      guardNote:
        '복구 가능한 작업은 기본 허용; rm -rf / sudo / 강제 푸시 등 고위험 명령은 차단; 작업 디렉터리가 git 저장소이면 실행 전 스냅샷을 만들어 되돌릴 수 있습니다.',
      status: '상태',
      detect: '감지',
      detecting: '감지 중…',
      ...
      notInstalledHint:
        '먼저 Claude Code 를 설치하세요(docs.anthropic.com/claude-code 참고). 또는 아래에 실행 파일 전체 경로를 입력하세요.',
      mcpServers: 'MCP 서버 {{count}}개 구성됨',
      computerUsePresent: '데스크톱 제어(computer use) MCP 구성됨',
      computerUseAbsent:
        '데스크톱 제어 MCP 없음(복사/붙여넣기 같은 가벼운 작업은 Bash 로 가능, 불필요)',
      exePath: '실행 파일',
      workdir: '작업 디렉터리',
      workdirDesc:
    ...
    codingAgent: {
      ...
      opencodeMissing:
        'opencode 명령을 찾을 수 없습니다. 먼저 설치(npm i -g opencode-ai)하고 opencode auth login으로 로그인하세요.',
      cliReady: '{{name}} v{{version}}을(를) 감지했습니다.',
      cliMissing:
        '{{name}} 명령을 찾을 수 없습니다. 먼저 설치하고 로그인하거나, 아래 "실행 파일"에 절대 경로를 입력하세요.',
      sandboxGuardHint:
        '이 백엔드는 명령 단위 고위험 목록 없이 큰 단위의 샌드박스 등급만 제공합니다. 제한에 걸리면 "이 명령 승인" 카드를 띄우지 않고 실패를 그대로 알립니다.',
      codexModelHint:
        'Codex 모델 이름(예: gpt-5)을 입력하세요. 비워 두면 ~/.codex/config.toml 설정을 사용합니다.',
      codexBudgetHint:
        'Codex에는 실행별 달러 예산 상한이 없습니다. 비용은 구성한 제공자에 따라 달라집니다.',
      codexMode: {
        plan: '읽기 전용 / 계획',
      ...
      dshModelHint:
        'dsh의 headless 프로필에는 모델 전환이 없습니다. 모델은 dsh 자체 프로필에서 결정되며 여기서는 바꿀 수 없습니다.',
    ...
    debug: {
      ...
      cursorProbeDesc:
        '누른 뒤 카운트다운 안에 대상 앱으로 전환해 입력란을 클릭하세요. 그곳의 커서 주변 원문을 읽어, 어떤 앱이 읽히고 어떤 앱이 안전 게이트에 막히는지 확인할 수 있습니다. 한 번만 읽으며 어떤 제공자에게도 보내지 않습니다.',
    ...
    recording: {
      ...
      migrationNoticeDesc:
        '이전에 트리거 방식을 변경했다면 여기서 다시 한 번 확인해 주세요. 이번 업데이트는 단축키 방식의 기본값과 읽기 로직을 조정했습니다. "눌러서 말하기"가 더 익숙하다면 다시 전환할 수 있습니다.',
      ...
      microphoneDialogDesc:
        '목소리를 받을 수 있는 마이크를 선택하세요. 미터가 움직이지 않으면 다른 마이크를 시도하세요.',
      ...
      comboRecordDesc:
        '클릭 후 원하는 단축키 조합(예: ⌘⇧D)을 누르세요. 토글 및 누르기 모드 모두 지원합니다.',
      ...
      windowsInsertionModeDesc:
        '받아쓰기 결과를 커서 위치에 삽입하는 방법. 클립보드 붙여넣기는 위의 「붙여넣기 단축키」를 사용하며 줄바꿈을 유지합니다.',
      ...
      macosNewlineModeDesc:
        '자동은 알려진 터미널 앱에서 Line Feed(U+000A / Ctrl+J)를, 그 밖의 앱에서는 Shift+Return을 사용합니다. 일반 Return은 메시지를 전송합니다.',
      ...
      windowsSendInputNewlineModeDesc:
        'SendInput 모드에서 줄바꿈을 어떤 키로 보낼지. 채팅 입력창은 Shift+Enter, 메모장 / VS Code 등은 Enter.',
      ...
      windowsShowOpenlessInKeyboardListDesc:
        '끄면 Win+Space로 OpenLess에 전환되지 않습니다. SendInput 및 클립보드 붙여넣기 삽입에는 영향 없습니다. 다시 켜면 목록에 복원됩니다.',
      windowsShowOpenlessInKeyboardListError:
        '키보드 목록을 업데이트할 수 없습니다: 시스템이 OpenLess 언어 프로필 변경을 거부했습니다.',
      historyGroupTitle: '기록 및 컨텍스트',
      historyRetentionLabel: '기록 보관 기간(일)',
      historyRetentionDesc:
        '보관 기간을 초과한 기록은 새 항목 작성 시 정리됩니다. 0 = 시간 기반 정리 비활성화.',
      ...
      startMinimizedDesc:
        '모든 시작 경로에서 메인 창을 열지 않고 메뉴 막대 / 트레이에서만 실행합니다.',
    ...
    channels: {
      ...
      disabled: '사용 안 함',
      enabledFor: '{{name}} 사용',
      modelNotSet: '모델을 별도로 설정하지 않음',
      localModelManaged: '시스템 또는 로컬 모델 페이지에서 모델 관리',
      lastCheck: '마지막 확인',
      verifying: '확인 중…',
      ...
      failed: '확인 실패 · {{reason}}',
      elapsed: '소요 시간 {{ms}} ms',
      staleResult: '24시간이 지난 결과',
      connectionTitle: '서비스 연결',
      modelTitle: '모델 설정',
      modelHint: '모델 이름을 직접 입력하거나 공급자의 모델 목록을 가져와 선택하세요.',
      ...
      empty: '아직 채널이 없습니다. "채널 추가"로 첫 서비스를 연결하세요.',
      add: '채널 추가',
      edit: '편집',
      createTitle: '채널 추가',
      editTitle: '채널 편집',
      ...
      hoursAgo: '{{count}}시간 전',
      daysAgo: '{{count}}일 전',
      localEngineModelHint: 'AI 서비스 및 모델 → 로컬 모델에서 모델을 내려받고 전환할 수 있습니다.',
    },
    providers: {
      ...
      appleSpeechLocalNote:
        'Apple 음성 인식은 시스템 내장 엔진을 사용하므로 모델 선택이 필요 없습니다.',
      localEngineNote:
        '다운로드된 로컬 모델은 위의 드롭다운에서 바로 선택할 수 있습니다. 더 많은 모델은 「로컬 모델」에서 다운로드하고 관리합니다.',
      localTag: '로컬',
      llmTitle: 'LLM 모델(정리)',
      ...
      codexOAuthNotice:
        'Codex OAuth는 로컬 Codex 로그인 상태(~/.codex/auth.json)를 사용합니다. OpenLess는 API Key나 Base URL을 저장하지 않습니다.',
      asrProviderDesc: '전환 시 해당하는 자격 증명이 자동 선택됩니다.',
      asrTitle: 'ASR 음성(전사)',
      asrDesc: '녹음된 음성을 텍스트로 전사합니다.',
      omniTitle: '멀티모달 모델',
      omniDesc:
        '하나의 모델이 프롬프트 + 오디오를 받아 최종 텍스트를 한 번에 출력합니다(실험적 파이프라인).',
      ...
      elevenLabsUploadNotice:
        'ElevenLabs는 녹음 오디오를 설정된 엔드포인트에 업로드해 일괄 전사합니다.',
      zenmuxVocabularyNote:
        'ZenMux는 JSON 전사 프로토콜을 사용하며 사전 핫워드(prompt/hotwords)를 보내지 않습니다. 사전은 여전히 다듬기 단계에 전달되지만 음성 인식 편향에는 사용되지 않습니다.',
      asrAdvancedNote:
        '아래 고급 옵션은「커스텀 OpenAI 호환」및「ZenMux」프리셋에만 적용됩니다. 다른 명명된 공급자 프리셋은 내장 동작을 유지합니다.',
      asrAdvancedVerboseJsonLabel: '세그먼트 지표 (verbose_json)',
      asrAdvancedVerboseJsonHint:
        '서버가 지원할 때 환각 필터링용 segments 지표를 요청합니다. 지원하지 않는 자체 호스팅 서버에서는 꺼두세요.',
      asrAdvancedChunkLabel: '분할 시간 (ms)',
      asrAdvancedChunkHint:
        '0 = 분할 없음(전체를 한 번에 전송). 긴 녹음이나 요청당 시간 제한이 있는 서버에 적합합니다.',
      asrAdvancedEnableItnLabel: '숫자 정규화 (enable_itn)',
      asrAdvancedEnableItnHint:
        '구어 숫자/단위를 아라비아 숫자로 정규화합니다(예: "이천이십육" → "2026"). 끄면 원문을 유지합니다.',
      ...
      volcengineMappingNote:
        'Secret Key 는 현재 입력 불필요. Resource ID 기본값은 volc.seedasr.sauc.duration.',
      volcengineApiKeyNote:
        '새 음성 콘솔에서 만든 API Key로 인증하며 APP ID는 필요 없습니다. API Key는 음성 콘솔의 "API Key 관리"에서 생성합니다: console.volcengine.com/speech/new/setting/apikeys. Resource ID 기본값은 volc.seedasr.sauc.duration입니다.',
      ...
      asrProviderTakenOver:
        '현재 로컬 엔진을 사용 중입니다. 위의 드롭다운에서 다른 공급자를 선택하면 전환됩니다(로컬 엔진은 자동으로 중지됨). 로컬 모델은 「서비스 → 로컬 모델」에서 관리합니다.',
      ...
      localAsrPerformanceWarning:
        '로컬 추론은 클라우드보다 느리며 중국어 정확도가 낮을 수 있습니다. 오프라인 또는 개인정보 보호 시나리오에 적합.',
      ...
      messagesThinkingHint:
        '이전 모델이나 호환 게이트웨이는 고정 예산이 필요할 수 있습니다. 예산은 출력 한도보다 작아야 합니다. 사고 요청에는 온도를 보내지 않습니다.',
      ...
      thinkingModeHint:
        '선택한 요청 형식과 모델이 지원하는 매개변수로 사고를 켜거나 끄거나 줄입니다. 프롬프트에 제어 지시를 추가하지 않습니다.',
      ...
      bailianModelAsyncFileHint:
        '비동기 파일 모델 · 녹음을 업로드한 뒤 전사 작업 완료를 기다립니다.',
      ...
      endpointMustUseHttps:
        'HTTP Endpoint 를 사용할 수 있지만, API Key 와 음성 내용이 전송 중 유출될 수 있습니다.',
      endpointHttpWarning:
      ...
      qwen3EndpointSchemeInvalid:
        'Qwen3 실시간 ASR은 DashScope Realtime WebSocket 게이트웨이를 사용합니다. 엔드포인트는 wss://로 시작해야 합니다(기본값: wss://dashscope.aliyuncs.com/api-ws/v1/realtime). https:// 주소는 여기서 사용할 수 없습니다.',
    ...
    shortcuts: {
      ...
      androidReadOnly:
        'Android에서는 전역 단축키를 사용할 수 없습니다. 개요 페이지의 녹음 버튼을 사용하세요.',
    },
    permissions: {
      title: '권한',
      descAcc:
        'OpenLess 가 정상 작동하려면 다음 시스템 권한이 필요합니다. 허용 후에는 일반적으로 앱을 완전히 종료한 후 재시작해야 적용됩니다.',
      descNoAcc:
      ...
      hotkeyDescWithAdapter:
        '현재 어댑터: {{adapter}}. 단축키 감지가 설치되었는지 판정하기 위해 사용.',
      ...
      androidAccessibilityImpact:
        '켜면 키보드를 전환하지 않고 현재 입력칸에 결과를 출력합니다. 끄면 클립보드에 복사되며 직접 붙여넣어야 합니다.',
      ...
      androidAccessibilityMessages: {
        ...
        authorized_not_connected:
          '접근성은 승인되었지만 연결되지 않았습니다. 시스템 설정에서 OpenLess를 다시 활성화하세요.',
        status_read_failed: '접근성 상태를 읽을 수 없습니다.',
      },
      androidShizukuLabel: 'Shizuku 확장 모드',
      androidShizukuHint:
        '선택 기능. OEM 설정에서 수동 전환이 어려울 때 최선 노력 복구를 시도합니다. 앱 간 경합을 완전히 제거할 수는 없습니다. 재부팅 후 Shizuku를 다시 시작해야 할 수 있습니다.',
      ...
      androidShizukuRecoverConfirm:
        'Shizuku로 OpenLess 접근성 서비스를 다시 활성화할까요? 쓰기 시점에 활성화된 서비스는 병합됩니다. 전역 스위치가 꺼져 있으면 활성화 시 등록된 다른 서비스도 함께 시작될 수 있습니다.',
      androidShizukuYes: '예',
      ...
      androidShizukuMessages: {
        ...
        unsupported_backend:
          '이 Shizuku 백엔드는 너무 오래되었습니다. Shizuku 또는 Sui를 v11 이상으로 업데이트하세요.',
        ...
        permission_permanently_denied:
          'Shizuku 권한이 차단되었습니다. Shizuku를 열어 OpenLess를 수동으로 허용하세요.',
        ...
        partial_rollback:
          '복구에 실패했으며 설정은 일부만 되돌릴 수 있었습니다. 시스템 접근성 설정을 확인하세요.',
        manual_required:
          '전역 스위치가 꺼져 있고 다른 등록된 서비스가 있으면 자동 복구를 안전하게 수행할 수 없습니다. 시스템 설정에서 수동으로 진행하세요.',
      ...
      androidInsertStrategyHint: {
        accessibility: '접근성 서비스가 필요합니다. 사용할 수 없으면 클립보드에 복사합니다.',
        clipboard: '접근성 권한이 필요 없으며 직접 붙여넣습니다.',
      },
      ...
      androidOverlayTriggerHint: {
        background: '단순',
        keyboard: '이 모드는 보류되었습니다. 기존 설정은 백그라운드로 되돌립니다.',
        always: '항상 표시',
      },
      androidOverlayTriggerDisabled: {
        keyboard: '키보드 표시 감지는 보류되었습니다. 이후 오버레이 제스처로 대체합니다.',
      },
      androidOverlayActivationMode: { tap: '탭으로 활성화', long_press: '길게 눌러 활성화' },
      androidOverlayActivationModeHint: {
        tap: '첫 탭은 대기 상태로 전환하고, 두 번째 탭은 일반 받아쓰기를 시작합니다.',
        long_press:
          '누르고 있는 동안 대기 상태가 되며, 손을 떼면 현재 녹음 또는 QA 턴을 종료합니다.',
      },
      ...
      androidOverlayLeftSwipeActionHint: {
        ...
        style_pack: '대기 상태에서 왼쪽으로 밀면 이전 스타일 팩으로 전환합니다.',
      },
    ...
    theme: {
      ...
      stackedRowLayoutDesc:
        '작은 화면이나 큰 글꼴에서 한 줄에 맞지 않는 버튼과 옵션은 다음 줄로 넘어가 가로 넘침과 글자 눌림을 방지합니다.',
      conservativeLayoutLabel: '보수적 레이아웃',
      conservativeLayoutDesc:
        '홈, 상단 바, 하단 바 외 설정·기능 페이지를 단일 열·전체 너비로 표시하여 가로 넘침을 최대한 방지합니다.',
      system: '시스템 따르기',
      light: '라이트',
      dark: '다크',
    },
    ...
    about: {
      ...
      autoUpdateCheckDescAndroid:
        '시작 시 및 60분마다 확인합니다. 업데이트가 있으면 자동 다운로드 후 시스템 설치 프로그램을 엽니다. 채널은 위 Beta 스위치를 따릅니다.',
  ...
  startup: {
    loading: 'OpenLess 시작 중…',
    loadingDesc: '로컬 서비스에 연결하고 호환성을 확인하고 있습니다.',
    failed: 'OpenLess를 시작할 수 없습니다',
    recovery:
      '다시 확인하세요. 문제가 계속되면 앱을 완전히 종료한 후 다시 여세요. 업그레이드 후 발생한 경우 앱 전체가 동일한 버전인지 확인하세요.',
    retry: '다시 확인',
    details: '오류 세부 정보',
  },
  modal: {
    ...
    searchPlaceholder: '설정 카테고리 찾기…',
    clearSearch: '검색 지우기',
    categoriesLabel: '설정 카테고리',
    searchResults: '검색 결과',
    searchCount: '관련 카테고리 {{count}}개',
    ...
    advancedPages: {
      ...
      debug: '디버그 녹음을 보관하고 커서 문맥을 확인하며 로그를 내보냅니다.',
    },
    descriptions: {
    ...
    personalize: {
      ...
      blur: '서리유리 강도',
      blurDesc:
        '창 내부 backdrop-filter 강도에 영향(macOS 시스템 서리 레이어가 작동하지 않을 때 조정).',
    },
    about: {
      tagline: '자연스럽게 말하고, 정확하게 작성하세요',
      checkUpdate: '업데이트 확인',
      ...
      exportErrorLogDesc:
        '현재 세션의 실행 로그를 로컬에 저장합니다. 문제 조사나 피드백 전송에 사용하세요.',
      ...
      privacy: '프라이버시',
      privacyDesc:
        '인식 결과는 로컬에 저장되며 설정한 클라우드 공급자가 전사를 위해 녹음을 수신할 수 있습니다.',
      localFirst: '로컬 우선',
    },
  },
  windowChrome: {
  ...
  localAsr: {
    ...
    storageDesc:
      '사용자 지정 저장소는 선택한 폴더 아래에 OpenLess/models 를 만들고 기존 모델을 이동합니다. 이동 전에 다운로드를 취소하고 로드된 모델을 해제합니다.',
    storageChooseTitle: '로컬 모델 저장 상위 폴더 선택',
    storageChangeConfirm:
      '기존 로컬 모델을 {{path}}/OpenLess/models 로 이동합니다. 먼저 다운로드를 취소하고 로드된 모델을 해제합니다. 계속할까요?',
    storageResetConfirm:
      '기존 로컬 모델을 시스템 기본 폴더로 되돌립니다. 현재 폴더: {{path}}. 계속할까요?',
    modelDir: '모델 폴더',
    revealDir: '폴더 열기',
    deleteConfirm: '{{name}} 로컬 모델 파일을 삭제할까요? 되돌릴 수 없습니다.',
    appleSpeechTitle: 'Apple 음성 인식(macOS)',
    appleSpeechDesc:
      'macOS 기본 음성 인식을 사용해 로컬에서 음성을 텍스트로 변환합니다. 모델 다운로드, API 키, 네트워크가 모두 필요 없습니다. 클라우드 ASR이 불안정할 때 자격 증명이 필요 없는 로컬 폴백입니다. 처음 사용할 때 음성 인식 권한 요청이 표시됩니다.',
    appleSpeechUse: 'Apple 음성 사용',
    qwenTitle: 'Qwen3-ASR 모델 관리',
    qwenExperimentalBadge: '실험적',
    engineUnavailable:
      '현재 플랫폼에는 Qwen3-ASR 추론 엔진이 포함되어 있지 않습니다. 모델은 다운로드할 수 있지만 여기서는 아직 Qwen3-ASR 을 활성화할 수 없습니다.',
    qwenUnavailableOnWindows:
      'Windows 에서는 아직 Qwen3-ASR 을 지원하지 않습니다. 위의 Foundry Local Whisper 를 사용해 주세요.',
    ...
    foundryModelMediumDesc:
      '더 높은 정확도. 더 큰 다운로드와 느린 추론을 감당할 수 있는 고성능 기기에 적합합니다.',
    ...
    sherpaDesc:
      'Windows는 sherpa-onnx로 기기 내 오프라인 일괄 인식을 수행하며 ASR API 키가 필요 없습니다.',
    ...
    modelSelectTitle: '이 기기의 모델',
    modelSelectDesc: '다운로드 상태를 확인하고 파일을 관리하거나 모델을 불러와 테스트하세요.',
    ...
    performanceWarning:
      '로컬 ASR 은 오프라인 또는 개인정보 보호 시나리오에 적합. 첫 사용 시 모델 다운로드 필요.',
    test: '로드하여 테스트',
    testRunning: '테스트 중…',
    ...
    testActual: '인식',
    testStats:
      '오디오 길이 {{audio}}s · 로드 {{load}}s · 추론 {{transcribe}}s · 백엔드 {{backend}}',
    testFailed: '테스트 실패',
    ...
    downloadDialogDesc:
      '모델 크기와 설명을 확인하고 다운로드하세요. 완료되면 음성 인식에서 해당 로컬 서비스를 선택하세요.',
    detailRepo: '저장소',
    hfDownloads: '다운로드 수',
    hfLikes: '좋아요',
    hfDescription: '모델 소개',
    ...
    libraryEmptyTitle: '아직 로컬 모델이 없습니다',
    libraryEmptyDesc:
      '음성 인식 모델을 다운로드하면 이 기기에서 오디오를 처리할 수 있습니다. 기존 모델이 보이지 않으면 목록을 새로 불러오세요.',
    catalogTitle: '모델 카탈로그',
    catalogEmpty: '표시할 모델이 없습니다. 카탈로그를 새로 불러온 후 다시 시도하세요.',
    reloadCatalog: '목록 새로고침',
    engineLabel: '인식 엔진',
    sizeLabel: '모델 크기',
    allEngines: '전체',
    backToCatalog: '카탈로그로 돌아가기',
    detailsTitle: '모델 세부 정보',
    testActivateHint:
      '불러오기 및 테스트를 실행하면 이 모델을 현재 사용 모델로 설정한 후 내장 오디오 테스트를 진행합니다.',
    downloadProgressHint:
      '시작 후 모델 페이지에서 진행 상황을 확인하거나 다운로드를 취소할 수 있습니다.',
